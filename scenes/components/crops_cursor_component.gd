...
var corn_plant_seed = preload("res://scenes/objects/plants/corn.tscn")
var tomato_plant_seed = preload("res://scenes/objects/plants/tomato.tscn")
...
func add_crop() -> void:
	if distance < 20.0:
		...
		if ToolManager.selected_tool == DataTypes.Tools.PlantCorn:
			var corn_instance = corn_plant_seed.instantiate() as Node2D
		...
		if ToolManager.selected_tool == DataTypes.Tools.PlantTomato:
			var tomato_instance = tomato_plant_seed.instantiate() as Node2D
