...
func _unhandled_input(event: InputEvent) -> void:
	if in_range:
		if event.is_action_pressed("show_dialogue"):
			...
			#ballon.start(load("some dialogue script file here"), dialogue_start_command)
...
func trigger_feed_harvest(invetory_item: String, scene: Resource) -> void:
	var inventory: Dictionary = InventoryManager.inventory
	
	if !inventory.has(invetory_item):
		return
	
	var inventory_item_count = inventory[invetory_item]
	...
	for index in inventory_item_count:
		...
		# added to fix issues with it being overlaped by the chest
		harvest_instance.z_index += 2
		...
		InventoryManager.remove_collectable(invetory_item)
...
func add_reward_scene() -> void:
	for scene in output_reward_scenes:
		...
		var reward_position: Vector2 = GlobalFunctions.get_random_position_in_circle(reward_marker.global_position, reward_output_radius)
